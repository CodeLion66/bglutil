...
func TestBlock(t *testing.T) {
	b := bglutil.NewBlock(&Block100000)
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := bglutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := bglutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := bglutil.OutOfRangeError(wantErr)
	...
	_, err = bglutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(bglutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, bglutil.OutOfRangeError(""))
	}
