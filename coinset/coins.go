...
func (cs *CoinSet) TotalValue() (value bglutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue bglutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue bglutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount bglutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue bglutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue bglutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue bglutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue bglutil.Amount, coins []Coin) (Coins, error) {
...
// bglutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() bglutil.Amount {
	return bglutil.Amount(c.txOut().Value)
}
